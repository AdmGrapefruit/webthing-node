...
class CoapThingsHandler extends CoapBaseHandler {
    ...
    get(req, res) {
        const descriptions = (
            this.things.getThings().map(helpers.getDescription(thing))
        );
        res.end(helpers.getJson(res, descriptions));
    }
}
...
class CoapThingHandler extends CoapBaseHandler {
    ...
    get(req, res) {
        ...
        const description = helpers.getDescription(thing);
        res.end(helpers.getJson(res, description);
    }
}
...
class CoapCoresHandler extends CoapBaseHandler {
     ...
     */
    get(req, res) {
        const descriptions = this.things.getThings().map(
            helpers.getDescription(thing)
        );
        
        const cores = [];
        for (let d of descriptions) {
            core.push(getCoreLinkFormat(thing.asThingDescription()));
        }
...
class CoapCoreHandler extends CoapBaseHandler {
     ...
     */
    get(req, res) {
        const core = utils.getCoreLinkFormat(thing.asThingDescription());
...
class CoapPropertiesHandler extends CoapBaseHandler {
    ...
    get(req, res) {
        ...
        res.end(helpers.getJson(res, thing.getProperties()));
    }
}
...
class CoapPropertyHandler extends CoapBaseHandler {
    ...
    get(req, res) {
        ...
        if (thing.hasProperty(propertyName)) {
            res.end(helpers.getJson(res, {[propertyName]: thing.getProperty(propertyName)}));
        } else {
            res.statusCode = 404;
            res.end();
        }
...
class CoapActionsHandler extends CoapBaseHandler {
    ...
    post(req, res) {
        ...
        res.statusCode = 201;
        res.end(helpers.getJson(res, response));
    }
}
...
class CoapActionHandler extends CoapBaseHandler {
    ...
    get(req, res) {
        ...
        res.end(helpers.getJson(res, thing.getActionDescriptions(actionName)));
    }
...
class CoapActionIDHandler extends CoapBaseHandler {
    ...
    get(req, res) {
        ...
        res.end(helpers.getJson(res, action.asActionDescription()));
    }
...
class CoapEventsHandler extends CoapBaseHandler {
    ...
    get(req, res) {
        ...
        res.end(helpers.getJson(res, thing.getEventDescriptions()));
    }
}
...
class CoapEventHandler extends CoapBaseHandler {
    ...
    get(req, res) {
        ...
        res.end(helpers.getJson(res, thing.getEventDescriptions(eventName)));
    }
}
